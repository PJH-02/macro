...
|---|---|---|---|---|
| `G` | 한국 산업생산 YoY | 미국 산업생산 YoY | ECOS / KOSIS / FRED / ALFRED | 3개월 이동평균 YoY |
| `IC` | 한국 CPI YoY | 미국 CPI YoY | ECOS / KOSIS / FRED / ALFRED | 3개월 이동평균 + 목표대비 밴드 |
| `FC` | 한국 회사채 스프레드 | 미국 회사채 스프레드 | ECOS / FRED / ALFRED | 과거 대비 z-score |
| `ED` | 한국의 대미 수출 YoY | 미국 실질 재화수입 YoY | KOSIS / FRED / ALFRED | 3개월 이동평균 YoY |
| `FX` | USD/KRW | 광의 달러지수 | ECOS / 승인 FX 소스 / FRED / ALFRED | 3개월 log return |
...
|---|---|---|---|
| `KRX` | 시장/유니버스 소스 | 보통주 유니버스, 시장 overlay, KOSPI/KOSDAQ 종목 맥락 | active runtime provider |
| `DART` | 공시 소스 | Stage 2 공시 이벤트 | active runtime provider |
| `ECOS` | 한국 매크로/통계 소스 | CPI, 스프레드 등 한국 매크로 시계열 | active runtime provider path |
| `KOSIS` | 한국 매크로/통계 소스 | 한국 매크로 및 무역/수출 계열 시계열 | active runtime provider path |
| `FRED` | 미국 매크로 소스 | 최신/현재 미국 매크로 시계열 | active runtime provider path |
| `ALFRED` | 미국 역사 버전 소스 | revisable US series의 vintage-aware historical validation | active runtime provider path |
